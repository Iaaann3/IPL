                    ...
                    <tbody class="text-center">
                        @forelse($iklans as $iklan)
                            <tr>
                                ...
                                <td>
                                    @if($iklan->gambar)
                                        <img src="{{ asset('storage/' . $iklan->gambar) }}" alt="{{ $iklan->judul }}" class="img-thumbnail" style="max-width:100px;">
                                    @else
                                        -
                                    @endif
                                </td>
                                <td>
                                    ...
                                    <form action="{{ route('iklans.destroy', $iklan->id) }}" method="POST" class="d-inline-block" onsubmit="return confirm('Apakah Anda yakin ingin menghapus iklan ini?')">
                                        @csrf
                                        @method('DELETE')
